...
app.post('/api/scan', async (req, res) => {
  try {
    ...
    const scanMessages = [
      {
        role: 'system',
        content: `You are ClearIt, an image text reader and simplifier.
Output language: ${targetLang}
${targetLang === 'Burmese' ? 'You MUST write ONLY in Burmese script. No English in the text field.' : ''}
...
{"text":"your ${isSummarize ? 'summary' : 'simplified explanation'} in ${targetLang}","extracted":"raw text from image","lang":"detected language","topic":"one of: Tech Legal Science Health Finance Education Food Politics Philosophy Creative News Business Culture"}`
    ...
    const groqData = await groqRes.json();
    const raw = groqData.choices[0].message.content;
    console.log('Scanner RAW:', raw.substring(0, 300));

    const parsed = safeParseJSON(raw, 'text');
    if (!parsed) return res.status(500).json({ error: 'Could not read image. Try a clearer photo.' });
