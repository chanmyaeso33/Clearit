...
app.post('/api/simplify', async (req, res) => {
  try {
    ...
    // ✅ 9+ QUALITY PROMPT SYSTEM
    const langInstruction = targetLang === 'Burmese'
      ? `You MUST write ONLY in Burmese script (Myanmar Unicode \u1000-\u109F). No English words allowed.

BURMESE-SPECIFIC RULES (critical for naturalness):
- Use "ကမ္ဘာ့ရာသီဥတုပြောင်းလဲမှု" NOT "ရာသီဥတုပျက်စီးမှု" for climate change
- Use "ဖော့ဆီလ်လောင်စာ" NOT "ရေနံချေးများ" for fossil fuels
- Use "လေအား" NOT "လေရှား" for wind energy
- Use "စွမ်းအင်စနစ်" NOT "စွမ်းအင် အခြေပြု အဆောက်အအုံများ" for energy infrastructure
- Use "ရာသီဥတုပြောင်းလဲမှု" NOT "ရာသီဥတုပျက်စီးမှု"
- Write shorter clauses — avoid stacking too many nouns together
- Use spoken-form readability, not overly academic phrasing
- Prefer: "လူများ၏ လုပ်ဆောင်မှုများကြောင့်" over overly literal translations`
      : targetLang === 'Thai'
      ? `You MUST write ONLY in Thai script. No English words allowed.
- Use natural, simplified Thai — not formal academic Thai
- Break complex sentences into shorter Thai clauses
- Use everyday Thai vocabulary while preserving meaning`
      : `Write the output in ${targetLang}.`;
...
Step 2: Write a concise summary in ${targetLang} that captures all key information.
...
- For English: use "major" not "big" when describing large-scale changes

Your summary will be evaluated on:
- information completeness
- factual accuracy
- logical coherence
- fluency in ${targetLang}
- readability
...
{"text":"your complete summary in ${targetLang}","lang":"language of the input text","topic":"one of: Tech Legal Science Health Finance Education Food Politics Philosophy Creative News Business Culture"}`
      ...
      {
        role: 'system',
        content: `You are an expert multilingual text simplification system.
${langInstruction}
Respond ONLY with a valid JSON object. No markdown. No explanation. No backticks.`
      },
      {
...
Step 2: Rewrite in clear, natural, easy-to-understand ${targetLang} preserving everything from Step 1.

RULES:
- Preserve ALL important meaning, relationships, and nuance
- Replace difficult vocabulary with simpler but still PRECISE alternatives
- Use "major changes" not "big changes" for large-scale transformation
- Use shorter sentences when helpful
- Restore simultaneous relationships with words like "while also", "at the same time"
- Preserve "human-caused" or "caused by humans" when original says anthropogenic
- Preserve "economic inequality/disparities" — do not weaken to "economic differences"
- Do NOT oversimplify — never sacrifice precision for brevity
- Make it understandable for a high school student
- Write in natural, fluent ${targetLang}

Your simplification will be evaluated on:
- meaning preservation (most important)
- semantic accuracy — no drift from original meaning
- logical coherence — causal and simultaneous structure preserved
- fluency and naturalness in ${targetLang}
- clarity and readability
- nuance retention — technical terms simplified but not lost

TARGET QUALITY: near state-of-the-art educational simplification.
...
Return ONLY this JSON:
{"text":"your plain but precise simplified version in ${targetLang}","lang":"language of the input text","topic":"one of: Tech Legal Science Health Finance Education Food Politics Philosophy Creative News Business Culture"}`
      }
    ];
