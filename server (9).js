...
app.post('/api/scan', async (req, res) => {
  try {
    ...
    const extractMessages = [
      {
        role: 'system',
        content: `You are a text extraction system. Your ONLY job is to read text from images.
STRICT RULES:
- Copy ONLY the text that is physically visible in the image
- Do NOT summarize, explain, or interpret anything
- Do NOT add any information not present in the image
- Do NOT mention celebrities, brands, or names unless they are explicitly written in the image
- If a word is unclear or broken, write it as-is with [?] after it
- If a section is unreadable, write [unreadable section]
- Preserve the original language exactly as written
Return ONLY the raw extracted text. No JSON. No explanation. Just the text.`
      },
      {
        role: 'user',
        content: [
          ...
          {
            type: 'text',
            text: `Read every word visible in this image exactly as written.
- The text may be rotated or at an angle — read it from all directions
- Copy characters exactly, including any special scripts (Burmese, Thai, etc.)
- If text is damaged or blurry, mark it [?]
- Do NOT add anything that is not written in the image
Output ONLY the raw text from the image, nothing else.`
          }
        ]
      }
    ];
    ...
    const extractRes = await fetch('https://api.groq.com/openai/v1/chat/completions', {
      ...
      body: JSON.stringify({
        ...
        max_tokens: 1500,
        temperature: 0.1,  // Very low — we want exact copying, not creativity
    ...
    console.log('STEP 1 — Extracted text:', extractedText.substring(0, 400));
    ...
    const groundedMessages = [
      {
        role: 'system',
        content: `You are a grounded multilingual text simplification system.
${langInstruction}

ANTI-HALLUCINATION RULES — These are absolute. Never break them:
- Use ONLY information explicitly present in the SOURCE TEXT below
- Do NOT infer, guess, or add any details not written in the source
...
- Do NOT expand on topics based on your general knowledge
- If the source text is incomplete or marked [?] or [unreadable section], work only with what is clear
- If you cannot produce a faithful output from the available text, say: "The scanned text was too incomplete to summarize accurately."
- Source fidelity is more important than fluency — a shorter faithful output is better than a longer hallucinated one

Respond ONLY with a valid JSON object. No markdown. No explanation. No backticks.`
      },
      {
        role: 'user',
        content: `SOURCE TEXT (extracted from image — treat this as the ONLY source of truth):
"""
${extractedText}
"""

Task: ${isSummarize ? 'Summarize' : 'Simplify'} the SOURCE TEXT above in ${targetLang}.

${isSummarize
  ? `SUMMARIZE RULES:
- Write only what the source text explicitly states
- Preserve all names, numbers, facts exactly as written
- 3-5 sentences maximum
- Do NOT add context or background knowledge`
  : `SIMPLIFY RULES:
- Rewrite in plain, clear ${targetLang} that anyone can understand
- Keep every fact and meaning from the source
- Do NOT add examples or details not in the source
- Target: educated high school student`}

REMINDER: Never mention anything not written in the source text above.

Return ONLY this JSON:
{"text":"your grounded ${isSummarize ? 'summary' : 'simplified version'} in ${targetLang}","lang":"detected language of the source text","topic":"one of: Tech Legal Science Health Finance Education Food Politics Philosophy Creative News Business Culture"}`
      }
    ];
